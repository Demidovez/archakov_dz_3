function TodoItem(props) {
  const { id, text, color, completed } = props.task;
  ...
  const onEditText = () => {
    const newText = window.prompt("Измените текст", text);

    if (newText && newText !== text) {
      props.onEdit(id, { text: newText });
    }
  };
      ...
      <div className="todo-checkbox">
        <label htmlFor={id}>
          <input
            ...
            id={id}
            checked={completed}
            onChange={() => props.onEdit(id, { completed: !completed })}
          />
          <div>
